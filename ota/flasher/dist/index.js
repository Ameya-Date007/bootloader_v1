...
// Packet buffer
let packets = [];
let lastPacket = new Packet(1, Buffer.from([0xff]));
...
uart.on('data', data => {
    ...
    while (rxBuffer.length >= PACKET_LENGTH) {
        ...
        // console.log(raw);
        const packet = new Packet(raw.readUInt8(0), raw.subarray(1, 1 + PACKET_DATA_BYTES), raw[PACKET_CRC_INDEX]);
...
const main = async () => {
    ...
    */
    const firmwareFilename = "/home/ameya/ota/uploads/signed/signed.bin";
    Logger.info('Reading the firmware image...');
    const fwImage = await fs.readFile((firmwareFilename));
    ...
    const deviceId = fwImage.readUint32LE(FWINFO_DEVICE_ID_OFFSET);
